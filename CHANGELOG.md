...
### Security
- **`init` now blocks the full secret-store dump via `secretless-ai env`, and the generated guard hook's command inspection actually runs.** `secretless-ai env` prints every stored secret as plaintext `export` statements (it exists for the user's shell-profile `eval` hook). `secret get` was TTY-guarded and `run -- env` was already denied, but the bare `env` command had no deny rule, so an AI agent inside a "protected" project could exfiltrate the entire machine-global store with one documented command. A `Bash(*secretless-ai env*)` deny rule and a matching guard-hook arm now block it. **Root cause of a wider gap:** the guard hook's entire Bash-command branch was dead — every Bash tool call died at the `FILE_PATH` extraction under `set -euo pipefail` (a `grep` with no match returns non-zero) before reaching any command guard, so none of the hook's Bash protections (`cat .env`, `run -- env`, `secret get --force`, ...) ever fired. The JSON-field extractions now tolerate a no-match (`|| true`), reviving the whole branch. Native `permissions.deny` rules already enforced these at the Claude Code layer; this restores the hook as a working second layer. Found by release-test 2026-07-16. Re-run `secretless-ai init` to regenerate the hook and deny rules.
